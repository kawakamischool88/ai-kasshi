...
- Phase 3A（完了）：記憶候補 → 本人確認 → 確定。
  **確定記憶をAI回答へ再利用する機能はまだ作っていない。**
- **Phase 3A では作らなかった**：確定記憶の検索・ベクトル検索・AI回答への注入・出典表示画面・
  記憶の自動統合・矛盾の自動解決・記憶同士の関連付け・訂正・考えの更新・削除・PDF・
  人物像や性格の推測・柏村さん本人の実データ。
- Phase 3B / 3C（未着手）：**指示があるまで着手しない。**
